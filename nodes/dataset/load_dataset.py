...
def load(path, hasheader, target_col):
...
def drop_unimportant(X:pd.DataFrame, y):
...
@nodered_function()
def load_dataset(path, col, hasheader, encode, fillConstant, fillSelect):
    
    # Reading the data
    X, y = load(path, hasheader, col)
    print(f'Loaded dataset from {path}', f'X.shape is {X.shape}', f'y.shape is {y.shape}')

    # Dropping all nan rows and cols, all same cols
    X, y = drop_unimportant(X, y)

    # Encoding
    X, y = encoding(X, y, encode)
